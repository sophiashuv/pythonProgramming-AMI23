...
def user_arr():
    ...
    while True:
        try:
            mas = [int(i) for i in input("Enter numbers of array with whitespaces: ").split()]
        except ValueError:
...
def random_arr(n, low, height):
    ...
    """
    mas = [random.randint(low, height) for i in range(n)]
    return mas
...
def occurrence_of_each_elem(nums):
    ...
    >>> occurrence_of_each_elem([-8, 0, 3, 8, -1, -6, -9, -6, 6])
    {-8: [0], 0: [1], 3: [2], 8: [3], -1: [4], -6: [5, 7], -9: [6], 6: [8]}
    """
    occurrences = dict(map(lambda num: (num, [i for i in range(len(nums)) if nums[i] == num]), nums))
    return occurrences
...
def generate_new_array(list_array):
    ...
    >>> generate_new_array([-8, 0, 3, 8, -1, -6, -9, -6, 6])
    [0, 0, 0, 0, 0, 1, 0, 1, 0]
    """
